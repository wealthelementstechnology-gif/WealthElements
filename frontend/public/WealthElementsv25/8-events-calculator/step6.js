(function(){
  ...
  function updateOptimizeButtonState(totalSip) {
    ...
    const primaryActionIcon = document.getElementById('primaryActionIcon');
    const primaryActionText = document.getElementById('primaryActionText');
    const optimizeBtn = primaryActionBtn; // Keep reference for backward compatibility
    ...
    // If total SIP is within budget (with small buffer), switch to Next mode
    if (totalSip <= (budget + budgetBuffer)) {
      primaryActionBtn.setAttribute('data-mode', 'next');
      if (primaryActionIcon) primaryActionIcon.textContent = 'arrow_forward';
      if (primaryActionText) primaryActionText.textContent = 'Next';
      primaryActionBtn.title = 'Your plan fits within budget. Proceed to next step.';
      ...
      // If total SIP exceeds budget, keep in Optimize mode (if not at max attempts)
      if (window.optimizationCount < window.maxOptimizations) {
        primaryActionBtn.setAttribute('data-mode', 'optimize');
        const remaining = window.maxOptimizations - window.optimizationCount;
        if (primaryActionIcon) primaryActionIcon.textContent = 'tune';
        if (primaryActionText) primaryActionText.textContent = `Optimize SIP (${remaining} left)`;
        primaryActionBtn.title = 'Your plan exceeds your budget. Click to optimize.';
  ...
  function optimizePlan() {
    ...
    if (primaryActionBtn) {
      ...
      const remaining = window.maxOptimizations - window.optimizationCount;
      if (primaryActionIcon) primaryActionIcon.textContent = 'hourglass_empty';
      if (primaryActionText) primaryActionText.textContent = `Optimizing... (${remaining} left)`;
    }
    ...
    if (window.optimizationCount === 1 && !stepUpAlreadyApplied) {
      ...
      setTimeout(() => {
        ...
        const primaryActionBtn2 = document.getElementById('primaryActionBtn');
        const primaryActionIcon2 = document.getElementById('primaryActionIcon');
        const primaryActionText2 = document.getElementById('primaryActionText');

        if (primaryActionBtn2) {
          primaryActionBtn2.disabled = false;
          if (newTotal <= budget) {
            // Switch to Next mode
            primaryActionBtn2.setAttribute('data-mode', 'next');
            if (primaryActionIcon2) primaryActionIcon2.textContent = 'arrow_forward';
            if (primaryActionText2) primaryActionText2.textContent = 'Next';
          } else {
            // Keep in Optimize mode
            const remaining = window.maxOptimizations - window.optimizationCount;
            primaryActionBtn2.setAttribute('data-mode', 'optimize');
            if (primaryActionIcon2) primaryActionIcon2.textContent = 'tune';
            if (primaryActionText2) primaryActionText2.textContent = `Optimize SIP (${remaining} left)`;
          }
        }
    ...
    setTimeout(() => {
      ...
      const primaryActionBtn3 = document.getElementById('primaryActionBtn');
      const primaryActionIcon3 = document.getElementById('primaryActionIcon');
      const primaryActionText3 = document.getElementById('primaryActionText');

      if (primaryActionBtn3) {
        primaryActionBtn3.disabled = false;
        if (newTotal <= budget) {
          // Switch to Next mode
          primaryActionBtn3.setAttribute('data-mode', 'next');
          if (primaryActionIcon3) primaryActionIcon3.textContent = 'arrow_forward';
          if (primaryActionText3) primaryActionText3.textContent = 'Next';
        } else if (window.optimizationCount < window.maxOptimizations) {
          // Keep in Optimize mode
          const remaining = window.maxOptimizations - window.optimizationCount;
          primaryActionBtn3.setAttribute('data-mode', 'optimize');
          if (primaryActionIcon3) primaryActionIcon3.textContent = 'tune';
          if (primaryActionText3) primaryActionText3.textContent = `Optimize SIP (${remaining} left)`;
        } else {
          // Switch to Next mode when max attempts reached
          primaryActionBtn3.setAttribute('data-mode', 'next');
          if (primaryActionIcon3) primaryActionIcon3.textContent = 'arrow_forward';
          if (primaryActionText3) primaryActionText3.textContent = 'Next';
        }
    ...
    setTimeout(() => {
      ...
      const bufferStatus = budgetBuffer >= 0 ? 
        ...
        // Re-enable button or switch to Next mode if max reached
        const primaryActionBtn = document.getElementById('primaryActionBtn');
        const primaryActionIcon = document.getElementById('primaryActionIcon');
        const primaryActionText = document.getElementById('primaryActionText');
  ...
  // Add event listener for primary action button (switches between Optimize and Next)
  ...
  if (primaryActionBtn) {
    primaryActionBtn.addEventListener('click', function() {
      // Check if button is in "Next" mode
      const isNextMode = primaryActionBtn.getAttribute('data-mode') === 'next';

      if (isNextMode) {
        // Navigate to step 7
        navigateToStep7();
      } else {
        // Call optimize function through window.optimizePlan so ML integration can wrap it
  ...
  function resetOptimization() {
    ...
    // Switch button back to Optimize mode
    const primaryActionBtn = document.getElementById('primaryActionBtn');
    const primaryActionIcon = document.getElementById('primaryActionIcon');
    const primaryActionText = document.getElementById('primaryActionText');
    ...
    if (primaryActionBtn) {
      primaryActionBtn.setAttribute('data-mode', 'optimize');
      if (primaryActionIcon) primaryActionIcon.textContent = 'tune';
      if (primaryActionText) primaryActionText.textContent = 'Optimize SIP (3 left)';
    }
  ...
  // Function to navigate to Step 7
