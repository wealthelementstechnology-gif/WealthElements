(function () {
  ...
  function createNestedPieChart(ctx, detailedData) {
    ...
    const brightColors = {
      ...
      // Insurance - Purples for Term, Pinks for Health
      insuranceTerm: ['#8b5cf6', '#a78bfa', '#c4b5fd', '#ddd6fe'],
      insuranceHealth: ['#ec4899', '#f472b6', '#f9a8d4', '#fbcfe8'],
    ...
    const finalColors = allData.map((_, index) => {
      ...
      } else if (label.includes('Insurance Premiums')) {
        // Check if it's health or term insurance by looking at the data
        const insuranceIndex = allLabels.slice(0, index).filter(l => l.includes('Insurance Premiums')).length;
        const insuranceItem = detailedCashFlow['Insurance Premiums']?.data[insuranceIndex];
        const isHealth = insuranceItem?.type && insuranceItem.type.toLowerCase() === 'health';
        return isHealth
          ? brightColors.insuranceHealth[insuranceIndex % brightColors.insuranceHealth.length]
          : brightColors.insuranceTerm[insuranceIndex % brightColors.insuranceTerm.length];
  ...
  const detailedCashFlow = {
    ...
    'Insurance Premiums': {
      data: data.insurances.map(item => ({
        label: item.policy || 'Unnamed Insurance',
        value: convertInsuranceToMonthly(item.premium, item.mode),
        type: item.type // Store type for color coding
      })).filter(item => item.value > 0),
      color: '#8b5cf6' // Purple for insurance
    },
    // Investments
    'Investments': {
  ...
  function getThemeColors() {
    ...
    return {
      ...
      headerColor: isDark ? '#00FF7F' : '#16a34a',
  ...
  function createCashFlowLegend(detailedData) {
    ...
    Object.entries(detailedData).forEach(([category, categoryData]) => {
      if (categoryData && categoryData.data && Array.isArray(categoryData.data) && categoryData.data.length > 0) {
        ...
        } else if (category === 'Insurance Premiums') {
          categoryColor = '#8b5cf6'; // Purple for insurance category header
        ...
        categoryData.data.forEach((item, index) => {
          if (item && item.label && typeof item.value === 'number' && item.value > 0) {
            ...
            if (category === 'Income Sources') {
              ...
              const isHealth = item.type && item.type.toLowerCase() === 'health';
              itemColor = isHealth
                ? brightColors.insuranceHealth[index % brightColors.insuranceHealth.length]
                : brightColors.insuranceTerm[index % brightColors.insuranceTerm.length];
            } else if (category === 'Investments') {
